...
func getSeasonEpisodes(contentId string) []SeasonEpisode {
	req, err := http.NewRequest(http.MethodGet, fmt.Sprintf("https://www.crunchyroll.com/content/v2/cms/seasons/%s/episodes?preferred_audio_language=en-US&locale=en-US", contentId), nil)
	if err != nil {
		panic(err)
	}
...
func getSeasons(contentId string) []Season {
	req, err := http.NewRequest(http.MethodGet, fmt.Sprintf("https://www.crunchyroll.com/content/v2/cms/series/%s/seasons?force_locale=&preferred_audio_language=en-US&locale=en-US", contentId), nil)
	if err != nil {
		panic(err)
	}
	req.Header.Set("Authorization", "Bearer "+token)
