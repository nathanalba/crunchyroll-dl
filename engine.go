...
func getFilename(set *mpd.AdaptationSet) string {
	if set == nil {
		f, _ := os.CreateTemp("/mnt/steam/crunchyroll/tmp", "crdl-subs-*.ass")
		return f.Name()
	}
	for _, representation := range set.Representations {
		if representation.Height != nil {
			f, _ := os.CreateTemp("/mnt/steam/crunchyroll/tmp", "crdl-video-*.mp4")
			name := f.Name()
			f.Close() // close it so downloadParts can reopen it
			return name
		} else if representation.Bandwidth != nil {
			f, _ := os.CreateTemp("/mnt/steam/crunchyroll/tmp", "crdl-audio-*.mp3")
			name := f.Name()
			f.Close()
			return name
		}
...
func downloadParts(baseUrl, representationId *string, set *mpd.AdaptationSet) (string, error) {
	...
	// Write directly to a temp file instead of holding everything in RAM
	tmpFile, err := os.CreateTemp("/mnt/steam/crunchyroll/tmp", "crdl-raw-*.mp4")
	if err != nil {
		return "", err
	}
	...
	initData = nil // free immediately

	// Download segments in order, writing each to disk as it arrives
	...
	// Write segments to disk in order, freeing each after writing
	for i, data := range results {
		if _, err := tmpFile.Write(data); err != nil {
			os.Remove(tmpName)
			return "", err
		}
		results[i] = nil // free segment immediately after writing
	}

	fmt.Println("\nFinished downloading!")

	// Now decrypt from the temp file into the output file
	filename := getFilename(set)
	outFile, err := os.Create(filename)
	if err != nil {
		//fmt.Printf("Raw temp: %s\nDecrypted output: %s\n", tmpName, filename)
		os.Remove(tmpName)
		return "", err
	}
	defer outFile.Close()
	...
	tmpFile.Seek(0, 0)
	if err = widevine.DecryptMP4Auto(io.NopCloser(tmpFile), keys, outFile); err != nil {
		os.Remove(tmpName)
		return "", fmt.Errorf("widevine.DecryptMP4Auto: %w", err)
	}

	os.Remove(tmpName) // clean up raw temp file
	return filename, nil
}
...
func downloadEpisode(contentId string, videoQuality, audioQuality, subtitlesLang *string, info EpisodeInfo) {
	...
	manifest, rawXML := parseManifest(episode.ManifestURL)
	pssh := getPssh(manifest, rawXML)
	if pssh == nil {
	...
	err := getLicense(*pssh, episodeId, episode.Token)
	if err != nil {
...
func processUrl(url, vq, aq, al, sl string) {
	...
	if contentType == "watch" {
		...
		downloadEpisode(contentId, &vq, &aq, &sl, info)
	} else {
		seasons := getSeasons(contentId)
		//fmt.Printf("No season number specified, downloading all seasons...\n")
		for _, season := range seasons {
			episodes := getSeasonEpisodes(season.ID)
			downloadSeason(&vq, &aq, &sl, episodes)
		}
	}
}
