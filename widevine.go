...
func getPssh(m *mpd.MPD, rawXML []byte) *string {
	// Match the cenc:pssh inside the Widevine ContentProtection block
	re := regexp.MustCompile(`(?s)edef8ba9-79d6-4ace-a3c8-27dcd51d21ed"[^>]*>.*?<cenc:pssh>(.*?)</cenc:pssh>`)
	matches := re.FindSubmatch(rawXML)
	if matches != nil {
		s := string(matches[1])
		return &s
	}
